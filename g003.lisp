...
(defstruct population-individual
	individual
	fitness)
	
(defun cost(individual)
	(let ((custo 0))
		(loop for i from 0 to (- (list-length individual) 2) do
			(setf custo (+ custo (aref distancias (nth i individual) (nth (1+ i) individual)))))
		custo))

(defun atsp-estado-inicial (tabuleiro)
  (let ((ini
  (make-atsp-estado :nos-visitados '(0)
                    :nos-nao-visitados (preenche-nao-visitados tabuleiro)
                    :no-atual 0
                    :custo-percorrido 0)))
    (setf distancias tabuleiro)
    ini))
    
(defun preenche-nao-visitados (tabuleiro)
	(let ((res))
		(loop for i from 1 to (1- (car (array-dimensions tabuleiro)))
			do (push i res))
	res))
    

(defun atsp-objectivo? (estado)
  (and (> (list-length (atsp-estado-nos-visitados estado)) 1) (equalp (find 0 (cdr (atsp-estado-nos-visitados estado))) 0)))
  

(defun atsp-operator (estado)
	(let ((estados))
		(if (equalp (atsp-estado-nos-nao-visitados estado) nil) (push 0 (atsp-estado-nos-nao-visitados estado)))
		(loop for i from 0 to (1- (list-length (atsp-estado-nos-nao-visitados estado)))
			do (push (make-atsp-estado :nos-visitados (append (atsp-estado-nos-visitados estado) (list (nth i (atsp-estado-nos-nao-visitados estado))))
									   :nos-nao-visitados (remove (nth i (atsp-estado-nos-nao-visitados estado)) (atsp-estado-nos-nao-visitados estado))
									   :no-atual (nth i (atsp-estado-nos-nao-visitados estado))
									   :custo-percorrido (+ (atsp-estado-custo-percorrido estado) (aref distancias (atsp-estado-no-atual estado) (nth i (atsp-estado-nos-nao-visitados estado))))) estados))
	estados))


(defun atsp-h (estado)
	(atsp-estado-custo-percorrido estado))
	
(defun atsp-h2(estado)
	(let ((menor1) (menor2) (soma 0) (n (car (array-dimensions distancias))))
		(dolist (no (append (atsp-estado-nos-nao-visitados estado))) (progn
			(setf menor1 (aref distancias no 0))
			(setf menor2 (aref distancias no 1))
			(loop for i from 0 to (1- n) do (cond
				((and (< (aref distancias no i) menor2) (< (aref distancias no i) menor1)) (progn (setf menor2 menor1) (setf menor1 (aref distancias no i))))
				((and (< (aref distancias no i) menor2) (> (aref distancias no i) menor1)) (setf menor2 (aref distancias no i)))
				((< (aref distancias no i) menor1) (progn (setf menor2 menor1) (setf menor1 (aref distancias no i))))))
			(setf soma (+ soma (+ menor1 menor2)))))
		(/ soma 2)))
	

(defun sucessor-menor-custo (sucessores)
	(let ((minim (first sucessores)))
	(dolist (suc sucessores)
		(if (< (atsp-estado-custo-percorrido suc) (atsp-estado-custo-percorrido minim))  (setf minim suc)))
	minim))
...
(defun nearest-neighbour (prob)
	(let* ((init (atsp-estado-inicial prob)) (sucessores) (estado-escolhido init))
		(loop while (not (atsp-objectivo? estado-escolhido)) do (progn 
			(setf sucessores (atsp-operator estado-escolhido))
			(setf estado-escolhido (sucessor-menor-custo sucessores))))
		(atsp-estado-nos-visitados estado-escolhido)))

		
#| ---------------------------- ITERATIVE SAMPLING ----------------------------------------------------|#

(defun iterative-sampling (prob)
	(let* ((distancias prob) (best-path) (current-path) (n (car (array-dimensions distancias))) (cont 0))
		(setf best-path (choose-random-path))
		(loop while (< cont (* n n n)) do (progn
			(setf current-path (choose-random-path))
			(if (> (fitness-function current-path) (fitness-function best-path)) (setf best-path current-path))
			(setf cont (1+ cont))))
	best-path))
...
(defun choose-random-path ()
	(let ((path '(0)) (unvisited-nodes) (rand) (n (car (array-dimensions distancias))))
		(loop for i from 1 to (1- n) do
			(push i unvisited-nodes))
		(loop while (not (equalp unvisited-nodes nil)) do (progn
			(setf rand (random (list-length unvisited-nodes)))
			(setf path (append path (list (nth rand unvisited-nodes))))
			(setf unvisited-nodes (remove (nth rand unvisited-nodes) unvisited-nodes))))
		(setf path (append path (list 0)))
	path))


(defun populate ()
	(let* ((firstOne (nearest-neighbour distancias)) (initial-population (list (make-population-individual :individual firstOne :fitness (fitness-function firstOne)))) (path))
		(loop for i from 2 to 10000 do (progn
			(setf path (choose-random-path))
			(push (make-population-individual :individual path :fitness (fitness-function path)) initial-population)))
		initial-population))
		
(defun fitness-function (individual)
	(let ((custo 0))
		(loop for i from 0 to (- (list-length individual) 2) do
			(setf custo (+ custo (aref distancias (nth i individual) (nth (1+ i) individual)))))
	(- 9999 custo)))
...
(defun mutate (individual)
	(let* ((mutant (remove 0 (population-individual-individual individual))) (pos1 (random (list-length mutant))) (pos2 (random (list-length mutant))))
		(loop while (equal pos1 pos2) do (setf pos2 (random (list-length mutant))))
		(rotatef (nth pos1 mutant) (nth pos2 mutant))
		(push 0 mutant)
		(setf mutant (append mutant (list 0)))
		(setf mutant (make-population-individual :individual mutant :fitness (fitness-function mutant)))
	mutant))
...
(defun validNext (sublist child)
	(let (res)
	(loop for i from 0 to (1- (list-length sublist)) do
		(if (not (member (nth i sublist) child)) (progn
		(setf res (nth i sublist))
		(return-from validNext res))))))
	

(defun reproduce (father mother)
	(let* ((valid1) (valid2) (parent1 (population-individual-individual father)) (child-fitness) (parent2 (population-individual-individual mother)) (sublist1) (sublist2) 
		(i1 0) (i2 0) (current2 0) (current1 0) (child) (n (car (array-dimensions distancias))))
	(setf child '(0))
	(loop while (not(equalp (list-length child) n)) do (progn
		(loop for i from 0 to (- (list-length parent1) 2) do (progn
			(if (equalp (nth i parent1) (first child)) (progn
				(setf current1 (nth i parent1)) 
				(setf i1 i)))
			(if (equalp (nth i parent2) (first child)) (progn
				(setf current2 (nth i parent2))
				(setf i2 i)))))
	   (setf sublist1 (subseq parent1 (1+ i1) (list-length parent1)))
	   (setf sublist2 (subseq parent2 (1+ i2) (list-length parent2)))
	   (if (or (equalp sublist1 nil) (equalp (validNext sublist1 child) nil)) (setf sublist1 (createList n)))
	   (if (or (equalp sublist2 nil) (equalp (validNext sublist2 child) nil)) (setf sublist2 (createList n)))
	   (setf valid1 (validNext sublist1 child))
	   (setf valid2 (validNext sublist2 child))
	   (if (< (aref distancias current1 valid1) (aref distancias current2 valid2)) (push valid1 child) (push valid2 child))))
   (push 0 child)
   (setf child (reverse child))
   (setf child-fitness (fitness-function child))
   (setf child (make-population-individual :individual child :fitness child-fitness))
   (if (and (> child-fitness (population-individual-fitness father)) (> child-fitness (population-individual-fitness mother)))
		(return-from reproduce child))
   (if (and (> (population-individual-fitness father) (population-individual-fitness mother)))
		(return-from reproduce father) (return-from reproduce mother))))

(defun random-selection (population)
	(let* ((soma-fitness 0) (rand) (soma-atual 0))
		(dolist (el population)
			(setf soma-fitness (+ soma-fitness (population-individual-fitness el))))
		(setf rand (random soma-fitness))
		(dolist (el population) (progn 
			(setf soma-atual (+ soma-atual (population-individual-fitness el)))
			(if (> soma-atual rand) (return-from random-selection el)))))) 

           
(defun best-individual (population)
	(let ((maxim (first population)))
	(dolist (el population)
		(if (> (population-individual-fitness el) (population-individual-fitness maxim))  (setf maxim el)))
	maxim))
	
		
(defun allEqual (population)
 (let* ((fst (first population)) (onlyOne (population-individual-individual fst)))
 (dolist (el population)
  (if (not (equal (population-individual-individual el) onlyOne)) (return-from allEqual nil)))
 (return-from allEqual t)))
 
 (defun medium(prob)
  (let ((somador 0) (x 0) (res))
  (loop for i from 1 to 100 do (progn
	(setf somador (+ somador (genetic-algorithm prob)))
	(setf x i)))
	(setf res (/ somador x))
	res))
  
(defun genetic-algorithm (prob)
 (let* ((currentBest (make-population-individual :individual '() :fitness 0)) (distancias prob) (bestIndividual) (n 0) 
        (size (* (car (array-dimensions distancias)) 3)) (population) (new-population) (parent1) (parent2) (child) (generation 0))
  (setf population (populate))
  (loop while (< generation size) do (progn
   (setf new-population '())
   (loop for i from 1 to 250 do (progn
    (setf parent1 (random-selection population))
    (setf parent2 (random-selection population))
    (setf child (reproduce parent1 parent2))
    (if (<= (random 1.0) 0.01) (setf child (mutate child)))
    (push child new-population)))
   (setf population new-population)
   (setf bestIndividual (best-individual population))
   (if (< (population-individual-fitness currentBest) (population-individual-fitness bestIndividual)) (setf currentBest bestIndividual)) 
   (setf n (1+ n))
   (if (allEqual population) (return-from genetic-algorithm (population-individual-individual currentBest)))
   (setf generation (1+ generation))))
  (return-from genetic-algorithm (population-individual-individual currentBest)))) 
		
#| ------------------------------------------------------ INTERFACE ------------------------------------------------------- |#

	
(defun atsp (prob strategy)
	(let ((estados1) (estados))
		(cond ((equalp strategy "a*.best.heuristic") (setf estados (car (procura (cria-problema (atsp-estado-inicial prob) (list #'atsp-operator) 
															:objectivo? #'atsp-objectivo? 
															:estado= #'equalp 
															:heuristica #'atsp-h) 
								  "a*" :espaco-em-arvore? T))))
			   ((equalp strategy "a*.best.alternative.heuristic") (setf estados (car (procura (cria-problema (atsp-estado-inicial prob) (list #'atsp-operator) 
															:objectivo? #'atsp-objectivo? 
															:estado= #'equalp 
															:heuristica #'atsp-h2) 
								  "a*" :espaco-em-arvore? T))))
			   ((equalp strategy "iterative.sampling") (return-from atsp (iterative-sampling prob)))
			   ((equalp strategy "best.approach") (return-from atsp (genetic-algorithm prob)))
			   (t (setf estados (car (procura (cria-problema (atsp-estado-inicial prob) (list #'atsp-operator) 
															:objectivo? #'atsp-objectivo? 
															:estado= #'equalp 
															:heuristica #'atsp-h) 
								  strategy :espaco-em-arvore? T)))))
		(dolist (e (reverse estados)) (push (atsp-estado-nos-visitados e) estados1)) ;preencher a lista de estados a retornar
		(car (reverse estados1))))
